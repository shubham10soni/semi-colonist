import React from "react";

const icons = () => {
  return (
    <div>
      <ul className="flex flex-row justify-start  space-x-3 laptop:space-x-4 ">
        <li>
          <a
            href="https://twitter.com/semi_colonised"
            className="text-gray-500 hover:text-gray-900 dark:hover:text-stone-900 dark:text-gray-500"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 -4 48 48"
              width={28}
              height={28}
            >
              <title>{"Twitter-color"}</title>
              <path
                fill="#00AAEC"
                fillRule="evenodd"
                d="M48 4.735a19.235 19.235 0 0 1-5.655 1.59A10.076 10.076 0 0 0 46.675.74a19.395 19.395 0 0 1-6.257 2.447C38.627 1.225 36.066 0 33.231 0c-5.435 0-9.844 4.521-9.844 10.098 0 .791.085 1.56.254 2.3C15.456 11.974 8.2 7.96 3.34 1.842A10.281 10.281 0 0 0 2.01 6.925c0 3.502 1.738 6.593 4.38 8.405a9.668 9.668 0 0 1-4.462-1.26v.124c0 4.894 3.395 8.977 7.903 9.901a9.39 9.39 0 0 1-2.595.356c-.634 0-1.254-.061-1.854-.18 1.254 4.01 4.888 6.932 9.199 7.01-3.37 2.71-7.618 4.325-12.23 4.325-.795 0-1.58-.047-2.35-.139C4.359 38.327 9.537 40 15.096 40c18.115 0 28.019-15.385 28.019-28.73 0-.439-.009-.878-.026-1.308A20.211 20.211 0 0 0 48 4.735"
              />
            </svg>
          </a>
        </li>

        <li>
          <a
            href="https://github.com/shubham10soni"
            className="text-gray-500 hover:text-gray-900 dark:hover:text-stone-900 dark:text-gray-500"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
              width={28}
              height={28}
              className="rounded-full dark:text-zinc-100 dark:fill-white shadow-md"
            >
              <title>{"github [#142]"}</title>
              <path
                fillRule="evenodd"
                d="M10 0c5.523 0 10 4.59 10 10.253 0 4.529-2.862 8.371-6.833 9.728-.507.101-.687-.219-.687-.492 0-.338.012-1.442.012-2.814 0-.956-.32-1.58-.679-1.898 2.227-.254 4.567-1.121 4.567-5.059 0-1.12-.388-2.034-1.03-2.752.104-.259.447-1.302-.098-2.714 0 0-.838-.275-2.747 1.051A9.396 9.396 0 0 0 10 4.958a9.375 9.375 0 0 0-2.503.345C5.586 3.977 4.746 4.252 4.746 4.252c-.543 1.412-.2 2.455-.097 2.714-.639.718-1.03 1.632-1.03 2.752 0 3.928 2.335 4.808 4.556 5.067-.286.256-.545.708-.635 1.371-.57.262-2.018.715-2.91-.852 0 0-.529-.985-1.533-1.057 0 0-.975-.013-.068.623 0 0 .655.315 1.11 1.5 0 0 .587 1.83 3.369 1.21.005.857.014 1.665.014 1.909 0 .271-.184.588-.683.493C2.865 18.627 0 14.783 0 10.253 0 4.59 4.478 0 10 0"
              />
            </svg>
          </a>
        </li>

        <li>
          <a
            href="http://www.linkedin.com/in/shubham-soni-716b93211"
            target="_blank"
            rel="noopener noreferrer"
          >
            <svg
              viewBox="0 0 32 32"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              width={28}
              height={28}
              className="bg-white dark:bg-black rounded-md shadow-md dark:fill-white"
            >
              <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <title>linkedin</title>
                <path d="M28.778 1.004h-25.56c-0.008-0-0.017-0-0.027-0-1.199 0-2.172 0.964-2.186 2.159v25.672c0.014 1.196 0.987 2.161 2.186 2.161 0.010 0 0.019-0 0.029-0h25.555c0.008 0 0.018 0 0.028 0 1.2 0 2.175-0.963 2.194-2.159l0-0.002v-25.67c-0.019-1.197-0.994-2.161-2.195-2.161-0.010 0-0.019 0-0.029 0h0.001zM9.9 26.562h-4.454v-14.311h4.454zM7.674 10.293c-1.425 0-2.579-1.155-2.579-2.579s1.155-2.579 2.579-2.579c1.424 0 2.579 1.154 2.579 2.578v0c0 0.001 0 0.002 0 0.004 0 1.423-1.154 2.577-2.577 2.577-0.001 0-0.002 0-0.003 0h0zM26.556 26.562h-4.441v-6.959c0-1.66-0.034-3.795-2.314-3.795-2.316 0-2.669 1.806-2.669 3.673v7.082h-4.441v-14.311h4.266v1.951h0.058c0.828-1.395 2.326-2.315 4.039-2.315 0.061 0 0.121 0.001 0.181 0.003l-0.009-0c4.5 0 5.332 2.962 5.332 6.817v7.855z"></path>{" "}
              </g>
            </svg>
          </a>
        </li>
        <li>
          <a href={`mailto:${"s.s.1.0.0.4.9.7hubh@gmail.com"}`}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              aria-label="Gmail"
              viewBox="0 0 512 512"
              width={28}
              height={28}
              className="rounded-md shadow-md"
            >
              <rect width={512} height={512} fill="#fff" rx="15%" />
              <path fill="#4285f4" d="M158 391V249l-82-63v175q0 30 30 30" />
              <path
                fill="#ea4335"
                d="m154 248 102 77 102-77v-98l-102 77-102-77"
              />
              <path fill="#34a853" d="M354 391V249l82-63v175q0 30-30 30" />
              <path
                fill="#c5221f"
                d="m76 188 82 63v-98l-30-23c-27-21-52 0-52 26"
              />
              <path
                fill="#fbbc04"
                d="m436 188-82 63v-98l30-23c27-21 52 0 52 26"
              />
            </svg>
          </a>
        </li>
      </ul>
    </div>
  );
};

export default icons;
